  ...
  describe('toSQL', () => {
    ...
    it('handles various operators', () => {
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.CONTAINS, value: 'v' }],
        }),
      ).toBe("f LIKE '%v%'")
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.BEGINS_WITH, value: 'v' }],
        }),
      ).toBe("f LIKE 'v%'")
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.ENDS_WITH, value: 'v' }],
        }),
      ).toBe("f LIKE '%v'")
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.IN, value: [1, 2] }],
        }),
      ).toBe('f IN (1, 2)')
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.BETWEEN, value: [1, 10] }],
        }),
      ).toBe('f BETWEEN 1 AND 10')
      expect(
        toSQL({
          condition: 'AND',
          rules: [{ id: '1', field: 'f', operator: Operator.IS_EMPTY, value: null }],
        }),
      ).toBe('f IS NULL')
    })
  })
    ...
    it('handles nested groups', () => {
      const group: QueryBuilderGroup = {
        ...
        rules: [
          ...
          {
            condition: 'OR',
            rules: [{ id: '2', field: 'age', operator: Operator.LESS, value: 18 }],
          },
        ],
      }
      const mongo = toMongo(group)
      expect(mongo).toEqual({
        $and: [{ name: { $eq: 'John' } }, { $or: [{ age: { $lt: 18 } }] }],
      })
    })
